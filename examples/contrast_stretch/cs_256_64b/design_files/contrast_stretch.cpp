...
#pragma HLS INTERFACE axis port=image_out
	...
	// Images are 256x256, thus we need to store values that are at most
	// 256*256 = 2^16 in the hist array
	uint17 hist[256];
	uint17 cumulative_hist, low_perc_count, hi_perc_count;
	int i, j;
	int c = 0;
	int d = 255;
	uint8 data_in;
...
#pragma HLS PIPELINE
	...
	low_perc_count = uint17((1 * N_ROWS*N_COLS)/100);
	hi_perc_count = uint17((99 * N_ROWS*N_COLS)/100);
	cumulative_hist = 0;
